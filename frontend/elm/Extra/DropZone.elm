...
type Model image
    = Model
        ...
        , uid : Int
        , uploadImageEndpoint : UploadMethod image
        , bgColor : Color
        , color : Color
        }


type UploadMethod image
    = HttpUpload
        ({ file : File.File
         , tracker : String
...
init : { uploadImageEndpoint : UploadMethod image, bgColor : Color, color : Color } -> Model image
init opts =
    Model
...
update : InnerMsg image -> Model image -> ( Model image, Cmd (Msg image) )
